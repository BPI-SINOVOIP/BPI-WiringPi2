...
The source currently contains 48 effective internal Banana Pi model IDs and 234 detection names. These numbers describe code coverage, not complete hardware validation. A board can be detected and mapped while pull control, alternate functions, hardware PWM, expander GPIO or exact device nodes remain limited.
