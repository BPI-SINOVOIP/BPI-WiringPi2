...
| Status | Meaning |
| --- | --- |
| `implemented` | Detection, pin map and a selectable backend exist; at least local build evidence is recorded. |
| `alias` | A documented compatible board/carrier reuses an existing map/backend. |
| `limited` | Code exists, but hardware validation or capabilities such as pull/alt/PWM/expander access are incomplete. |
...
| `out-of-scope` | This Pi-style Linux header library is not the correct interface for the product. |
...
- Newer Allwinner: M4 Berry and M4 Zero (H618; hardware PWM remains limited).
...
This list says a code path exists. Read the per-board row in the [porting plan](https://github.com/BPI-SINOVOIP/BPI-WiringPi2/blob/bpi-legacy-io-porting/docs/banana-pi-io-porting-plan-20260601.md) for capability and validation limits.

## Newly code-backed, awaiting hardware validation

- BPI-SM10 now has four detection names, a 27-line J12 map traced through the official carrier schematic, and a K3 register variant of the SpacemiT backend. Published physical pin 18 remains unavailable. Local build passed; exact image identity, gpioinfo and safe hardware tests are still required.
- BPI-CanMV-K230D Zero now has four detection names, a 27-line JP1 map from the official table, and a source-backed K230D two-bank GPIO/IOMUX backend. Physical pin 16 remains unavailable because the official row has no GPIO number. Local build passed; real board behavior, edge/PWM and bus nodes remain unverified.
- OpenWrt One now has exact model/compatible detection, an 11-line CN7 mikroBUS map closed through the official KiCad and upstream MT7981 pinctrl data, and board-specific selection of the exact DTS GPIO base `0x11d00000`. CN7 pin 9 (`AN`) remains outside the digital backend. Local build passed; pull, edge/PWM, device nodes, permissions and real hardware remain unverified.

## Hardware-validation work

BPI-F4, BPI-SM10, BPI-CanMV-K230D Zero and OpenWrt One all remain `limited` until named production images and safe real-hardware digital-I/O logs exist. F4 and OpenWrt One pull are no-ops; SM10 and K230D pull logic is source-backed but untested; alternate functions, edge/PWM and bus device nodes are not claimed as hardware-validated.
