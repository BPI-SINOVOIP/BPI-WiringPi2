...
- GitHub Wiki: user guides, concepts, support policy, porting and validation workflows.
...
5. Build and capture exact hardware evidence.
6. Update `Board-Support-Matrix.md`, limitations and roadmap.
7. Run `python3 tools/audit-bpi-support.py --peer ../RPi.GPIO`.
8. Review files under `docs/wiki/`, then publish the same commit set to the Wiki repository.
...
GitHub Wikis are separate Git repositories. Only the default branch is rendered. The reviewed files in `docs/wiki/` are copied to `BPI-WiringPi2.wiki.git`; `docs/wiki/README.md` stays in the main repository as maintainer instructions.

Before publishing:

- run `python3 tools/audit-bpi-support.py`;
- verify every relative Wiki link;
- verify shell/C examples against the current branch;
